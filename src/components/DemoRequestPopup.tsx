...
const DemoRequestPopup = ({
  ...
  // Update the demo field when defaultDemo changes
  useEffect(() => {
    if (defaultDemo) {
      form.setValue("demo", defaultDemo);
    }
  }, [defaultDemo, form]);
  ...
  const handleSubmit = async (data: DemoRequestFormValues) => {
    setIsSubmitted(true);
    try {
      const success = await submitToGoogleForm(data, 'demoRequest');
      if (success) {
        onSubmit(data);
      } else {
                  ...
                  <Select onValueChange={field.onChange} value={field.value}>
                    ...
                    <SelectContent>
                      {availableDemos.map((demo, index) => (
                        <SelectItem key={index} value={demo}>
                          {demo}
                        </SelectItem>
                      ))}
                    </SelectContent>
