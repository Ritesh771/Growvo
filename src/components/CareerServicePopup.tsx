...
const careerServiceFormSchema = z.object({
  name: z.string().min(1, {
...
type CareerServiceFormValues = z.infer<typeof careerServiceFormSchema>;

interface CareerServicePopupProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: CareerServiceFormValues) => void;
  title: string;
  description: string;
  defaultInterest?: string;
}

const CareerServicePopup = ({
  isOpen,
  onClose,
  onSubmit,
  title,
  description,
  defaultInterest = ""
}: CareerServicePopupProps) => {
  const [isSubmitted, setIsSubmitted] = useState(false);

  const form = useForm<CareerServiceFormValues>({
    resolver: zodResolver(careerServiceFormSchema),
    defaultValues: {
      name: "",
      email: "",
      phone: "",
  ...
  // Update the interest field when defaultInterest changes
  useEffect(() => {
    if (defaultInterest) {
  ...
  const handleSubmit = async (data: CareerServiceFormValues) => {
    setIsSubmitted(true);
    try {
      const success = await submitToGoogleForm(data, 'careerService');
      if (success) {
        onSubmit(data);
      } else {
      ...
      console.error('Error submitting career service form:', error);
...
export default CareerServicePopup;
